...
export const COMPANY_COLORS = ["#3498db", "#e67e22", "#2ecc71", "#9b59b6", "#e74c3c", "#16a085"];
...
const PerJobBarChart = ({ data = [], sortIndex, setSortIndex, minScore = 7.5 }) => {
  ...
  const options = {
    ...
    title: {
      text: "Jobs with High-Scoring Matches (Score > 7.5)",
      align: 'left',
    },
    credits: {
      enabled: false,
    ...
    tooltip: {
      ...
      formatter: function () {
        ...
        return `
          ...
          <span style="color:${this.point.color}">●</span> Company: <b>${pointData.company}</b><br/>
          Matches: <b>${this.point.y}</b><br/>
          Avg. Score: <b>${pointData.score ? pointData.score.toFixed(1) : ''}</b><br/>
        ...
        point: {
          events: {
            click: function () {
              ...
              const company = sortedData[this.index] && sortedData[this.index].company;
              const url = `/admin/matches?companyName=${encodeURIComponent(company)}&job_title=${encodeURIComponent(jobTitle)}`;
              navigate(url);
            },
          },
        },
