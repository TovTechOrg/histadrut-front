...
const CompanyJobsPieChart = ({ data = [] }) => {
	// Aggregate by company from data
	const companyCounts = {};
	data.forEach(item => {
	...
	let colorMap = getCompanyColorMap(data);
	let pieData = aggData;
	if (aggData.length > 5) {
	...
	// Assign colors
	pieData = pieData.map(d => ({ ...d, color: colorMap[d.name] }));
		...
		const options = {
			...
			series: [{ name: 'Jobs', colorByPoint: true, data: pieData }],
			legend: { enabled: true },
